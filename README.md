...
#### ✨ 新功能
- **自动注入 Llama 3 chat template** — 检测 Llama 3 模型（vocab_size=128256）并自动注入标准 chat template
- **自动修复 eot_token_id** — 检测 GGUF 缺少 eot_token_id 时自动修复，解决模型无限生成不停止的问题
- **进度条原地刷新** — tqdm 进度条现在在同一行刷新，不再卡住等 100% 才显示

#### 🐛 Bug 修复
- **修复: UTF-8 解码错误** — 转换/量化输出包含二进制数据时不再崩溃
- **修复: 进度条缓冲问题** — 改为按块读取（chunk-based），支持 \r 覆盖刷新
- **修复: config.json 检测** — 之前错误读取 tokenizer_config.json，现在正确读取 config.json
- **修复: 量化步骤输出** — 量化步骤的输出读取也改为按块读取

#### 🔧 改进
- **模板改为 Llama 3 格式** — 从 ChatML 改为 Llama 3 格式（<|start_header_id|> + <|eot_id|>），兼容更多模型
- **日志输出更流畅** — 不再卡顿，实时显示
